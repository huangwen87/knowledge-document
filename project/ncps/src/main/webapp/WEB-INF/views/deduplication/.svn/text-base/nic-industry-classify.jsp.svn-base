        ...
        <div class="container-fluid">
					...
					<div class="box-content">
						<form class="form-vertical" id="searchForm" >
							<fieldset>
							  ...
							  <div class="control-group">
								<label class="control-label" for="dateStr">查询日期</label>
								<div class="controls">
							  ...
							  <div class="control-group">
								...
								<div class="controls">
								  <select id="industry" name="industry" class="input-medium">
        	...
        	<script type="text/javascript">
				...
				function initForm(){
			        	...
			        	//$("#industry").append("<option value='A'>A</option>");
			        }
			    ...
			    function getSelectData(){
			           $.ajax({                           
								   ...
								   for (var i = 0; i <len2; i++) {
									  $("#industry").append("<option value='"+jsonList2[i].name+"'>"+jsonList2[i].name+"</option>");     
							       }     							            
								 }
			           });
			           $("#industry").chosen();
				...
				function init(){
						$("#gridTable").jqGrid({
							url:"nic/industry/query?"+ $('#searchForm').serialize(),
							datatype : "json",
			                height: 'auto',
			                autowidth: true,
			                //2013/9/4 按王安宇要求去除测试值一列
			                colNames:[ '新闻ID','标题',/* '测试值', */'所属行业','公司','时间'],
			                colModel:[
			                          {name:'id',width:'50',index:'id'},                      
			                          {name:'title',width:'150',index:'title'},  
			                         /*  {name:'value',width:'150',index:'value'}, */                    
			                          {name:'industry',width:'50',index:'industry'},            
			                          {name:'company',width:'80',index:'company'},            
		        ...
		        function query(){
			        	...
			        	var data="nic/industry/query?"+ $('#searchForm').serialize();
			   ...
			   function queryContentById(rowId){
						...
						$( "#dialog p" ).empty();
						$.ajax({
							url:"nic/industry/queryContentById?id="+filterId,
							type:"GET",
							async:false,
							success:function(msg){
					    ...
					    $( "#dialog" ).dialog("option", { title: "新闻ID:"+filterId+"对应新闻，其所属行业--"+rowObj.industry });
						$( "#dialog" ).dialog( "open" );
					}
