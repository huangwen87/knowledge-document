$(document).ready(function(){
	...
	$('ul.main-menu li a').each(function(){
		if($($(this))[0].href==String(window.location)){
			...
			$(this).parents('ul').siblings().children('li:not(.nav-header)').hide();// 隐藏未选中列表
		}
	});
...
//		window.location.href=basePath+"main/"+this.id;
	});
	...
	//层级菜单伸缩
	$('ul.main-menu li.nav-header').toggle(function(){
...
//	    nextText:'后一月',
	...
	$('#content').on('click','.btn-setting',function(e){
		e.preventDefault();
		$('#myModal').modal('show');
	});
